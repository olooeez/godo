...
import (
	...
	"gitlab.com/olooeez/godo/internal/task"
)
...
func TaskManagerCli(tasksFilePath string) *cli.App {
	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			UsageText: "godo add TITLE DESCRIPTION",
			Action:    addCommand,
		},
		{
			Name:      "remove",
		...
		},
		{
			...
			ArgsUsage: "ID STATUS",
			UsageText: "godo update ID STATUS",
...
func listCommand(c *cli.Context) error {
	...
	if actualNumArgs != 0 {
		return fmt.Errorf("invalid number of arguments (expected 0, got %d). see 'godo list --help' for more info", actualNumArgs)
	}
...
func addCommand(c *cli.Context) error {
	...
	if actualNumArgs != 2 {
		return fmt.Errorf("invalid number of arguments (expected 2, got %d). see 'godo add --help' for more info", actualNumArgs)
	}

	title := c.Args().First()
	if _, err := strconv.Atoi(title); err == nil {
		return errors.New("expected TITLE to be a string, but got a int. see 'godo add --help' for more info")
	}

	description := c.Args().Get(1)
	if _, err := strconv.Atoi(description); err == nil {
		return errors.New("expected DESCRIPTION to be a string, but got a int. see 'godo add --help' for more info")
	}
...
func removeCommand(c *cli.Context) error {
	...
	if actualNumArgs != 1 {
		return fmt.Errorf("invalid number of arguments (expected 1, got %d). see 'godo remove --help' for more info", actualNumArgs)
	}

	id := c.Args().First()
	if _, err := strconv.Atoi(id); err == nil {
		return errors.New("expected ID to be a string, but got a int. see 'godo remove --help' for more info")
	}
...
func updateCommand(c *cli.Context) error {
	...
	if actualNumArgs != 2 {
		return fmt.Errorf("invalid number of arguments (expected 2, got %d). see 'godo update --help' for more info", actualNumArgs)
	}
